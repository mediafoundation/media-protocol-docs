...
Only authorized modifier makes sure that `tx.origin` is the same as `msg.sender` or that `msg.sender` is an authorized proxy. This is used to make sure that only the actual providers and clients or authorized proxies like the [Marketplace Helper](../periphery/marketplace-helper) can execute certain functions like `registerProvider`, `unregisterProvider`, `updateProvider`, `updateProviderStake`, `deleteOffer`, `createDeal`, and `createDeals`.

### onlyOwner

Only the deployer of the Marketplace contract can execute functions with this modifier. This is used to make sure that only the owner of the contract can execute functions like `setToken`, `setLPToken`, `setProtocolFeeTo`, `setProtocolFeeRate`, `authorizeProxy`, and `deAuthorizeProxy`.
