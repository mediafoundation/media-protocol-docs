...
- [`receive`](#receive): Allows receiving native coin.
- [`recoverNative`](#recovernative): Allows the owner to recover any native coins sent to this contract.
- [`recoverERC20`](#recovererc20): Allows the owner to recover any ERC20 tokens sent to this contract.
- [`recoverERC721`](#recovererc721): Allows the owner to recover any ERC721 tokens sent to this contract.
...
```solidity
function authorizeProxy(address _addr) external onlyRole(DEFAULT_ADMIN_ROLE) returns (bool);
```
...
```solidity
function deAuthorizeProxy(address _addr) external onlyRole(DEFAULT_ADMIN_ROLE) returns (bool);
```
