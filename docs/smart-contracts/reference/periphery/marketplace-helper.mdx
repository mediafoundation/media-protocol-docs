...
This contract acts as a helper for the `Marketplace` contract. It allows users to register and unregister as providers, add liquidity, create deals, and add balance using ERC20 tokens or ETH. It utilizes Uniswap's `ISwapRouter02` and `INonfungiblePositionManager` interfaces to interact with Uniswap V3 pools. 
